    ...
    static func generateExtension(source: inout Source, paramCount: Int) throws {
        ...
        source.add("""
            ...
            public extension ObjectReference {
                @available(*, noasync)
                func callSelf\(allGenerics.bracketedOrNone)(env: Environment? = nil, _ name: String\(inGenericsAsArgs)) throws -> Result\(allWheres.backspaceIfNotEmpty()) {
                    let env = env ?? storedEnvironment
                    let function: TypedFunction\(paramCount)\(allGenerics.bracketedOrNone) = try get(env: env, name)
                    return try function.call(env, this: self\(inGenericsAsCallArgs))
                }

                @available(*, noasync)
                func call\(allGenerics.bracketedOrNone)(env: Environment? = nil, _ name: String\(inGenericsAsArgs)) throws -> Result\(allWheres.backspaceIfNotEmpty()) {
                    let env = env ?? storedEnvironment
                    let function: TypedFunction\(paramCount)\(allGenerics.bracketedOrNone) = try get(env: env, name)
                ...
                @available(*, noasync)
                func callSelf\(inGenerics.bracketedOrNone)(env: Environment? = nil, _ name: String\(inGenericsAsArgs)) throws \(inWheres.backspaceIfNotEmpty()) {
                    let env = env ?? storedEnvironment
                    let function: TypedFunction\(paramCount)<Undefined\(commaSeparatedInGenerics.prefixCommaIfNotEmpty())> = try get(env: env, name)
                    try function.call(env, this: self\(inGenericsAsCallArgs))
                }

                @available(*, noasync)
                func call\(inGenerics.bracketedOrNone)(env: Environment? = nil, _ name: String\(inGenericsAsArgs)) throws \(inWheres.backspaceIfNotEmpty()) {
                    let env = env ?? storedEnvironment
                    let function: TypedFunction\(paramCount)<Undefined\(commaSeparatedInGenerics.prefixCommaIfNotEmpty())> = try get(env: env, name)
                ...
                func callSelf\(allGenerics.bracketedOrNone)(_ name: String\(inGenericsAsArgs)) async throws -> Result\(allWheres.backspaceIfNotEmpty()) {
                    try await withEnvironment { env in try self.callSelf(env: env, name\(inGenericsAsCallArgs)) }
                }

                func call\(allGenerics.bracketedOrNone)(_ name: String\(inGenericsAsArgs)) async throws -> Result\(allWheres.backspaceIfNotEmpty()) {
                    try await withEnvironment { env in try self.call(env: env, name\(inGenericsAsCallArgs)) }
                }
